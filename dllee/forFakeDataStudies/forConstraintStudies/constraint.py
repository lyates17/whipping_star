...
def calcChi2(pred_spec, obs_spec, covar, debug=False):
    ...
    ## Inputs:
    ##   pred_spec    predicted spectrum (python list or TVectorT<double>)
...
if __name__ == "__main__":
    ...
    spec_fname = 'set2_withTwoBins/sens_pred.SBNspec.root'
    covar_fname = 'set2_withTwoBins/total_sens_pred.SBNcovar.root'
    data_spec_fname = 'set2_withTwoBins/fakedata.SBNspec.root'
    ...
    # Run the constraint
    nue1, covar1 = runMBConstraint(spec_dict['1e1p'], spec_dict['1mu1p'], total_covar, data_spec_dict['1mu1p'], debug=True)
    nue2, covar2 = runNewConstraint(spec_dict['1e1p'], spec_dict['1mu1p'], total_covar, data_spec_dict['1mu1p'], debug=True)

    if True:
        print("spectra...")
        for i in range(spec_dict['1e1p'].GetNbinsX()):
            print(i,nue1[i],nue2[i],nue1[i]-nue2[i])
        print("covariance...")
        for i in range(spec_dict['1e1p'].GetNbinsX()):
            for j in range(i+1):
                print(i,j,covar1[i][j],covar2[i][j],covar1[i][j]-covar2[i][j])

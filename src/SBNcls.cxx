...
int SBNcls::CalcCLSWithData(int numMC, std::string tag, SBNspec* data){
  ...
  std::cout << "  CNP_Chi for H1:     " << data_results[6] << std::endl;
  ...
  std::cout << "  Pearson_Chi for H0: " << h0_results[3].m_nlower[1] << ", " << pval2sig2sided(h0_results[3].m_nlower[1]) << " sigma" << std::endl;
  std::cout << "  Pearson_Chi for H1: " << h0_results[4].m_nlower[1] << ", " << pval2sig2sided(h0_results[4].m_nlower[1]) << " sigma" << std::endl;
  std::cout << "  CNP_Chi for H0:     " << h0_results[5].m_nlower[1] << ", " << pval2sig2sided(h0_results[5].m_nlower[1]) << " sigma" << std::endl;
  std::cout << "  CNP_Chi for H1:     " << h0_results[6].m_nlower[1] << ", " << pval2sig2sided(h0_results[6].m_nlower[1]) << " sigma" << std::endl;
...
int SBNcls::makePlotsWithData(CLSresult &h0_result, CLSresult & h1_result, float data_result, std::string tag, int which_mode){
  ...
  bool draw_both = true;
  if (which_mode==0) draw_both = false;
  ...
  // Median sensitivity
  // ... people sometimes ask for this, so... fine
  if (draw_both) {
    ...
    std::string a_string;
    if (sens_pval>0.001) a_string = to_string_prec(sens_pval,3);
    else a_string = "10^{"+to_string_prec(log10(sens_pval),2)+"}";
    std::string whatsigma;
    if (sens_pval!=0) whatsigma = to_string_prec(sens_sigma,1)+"#sigma";
    else whatsigma = "inf #sigma";
    std::string med_details =  ("#splitline{Median sensitivity }{1-#beta(" +to_string_prec(0.5,3) + ") #alpha("+ a_string +" | "+whatsigma+ ")}");
    //std::cout << med_details << std::endl;
    med_text->DrawLatexNDC(0.875,0.4,med_details.c_str());
  }
  ...
  // if we were going to draw the other p-value, that would go here...
  ...
  for(int i=0; i<pvals.size(); i++){
    ...
    if(which_mode==0) sigma_val = pval2sig2sided(pvals[i]);
    ...
    std::string details = "p-value: "+pval_str+", "+sigma_str;
  ...
  cp->Write();	
  cp->SaveAs(("SBNfit_Cls_withData_"+tag+".pdf").c_str(),"pdf");	
  fp->Close();
  
  return 0;
  
} // end definition of makePlotsWithData
